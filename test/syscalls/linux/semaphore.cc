...
// If true, all fields in a are identical to corresponding fields in b with the
// exception that sem_ctime in a is not greater than sem_ctime in b, which means
// a is created before b.
bool CreatedBefore(struct semid_ds const& a, struct semid_ds const& b) {
  return a.sem_perm.__key == b.sem_perm.__key &&
         a.sem_perm.uid == b.sem_perm.uid && a.sem_perm.gid == b.sem_perm.gid &&
         a.sem_perm.cuid == b.sem_perm.cuid &&
         a.sem_perm.cgid == b.sem_perm.cgid &&
  ...
  for (int i = 0; i <= max_used_index; i++) {
    ...
    if (sem_ids.find(sem_id) != sem_ids.end()) {
      ...
      if (val == -1) {
        ...
        EXPECT_TRUE(CreatedBefore(ipc_stat_ds, ds));
      }
